...
public class WorkMakerIT extends ConfigurableMacBase {
  ...
  public void singleUnitSingleTarget() throws Exception {
    ...
    m.put(StatusSection.NAME, new Text(tableId.getUtf8Bytes()), StatusUtil.fileCreatedValue(timeCreated));
  ...
  public void singleUnitMultipleTargets() throws Exception {
    ...
    Mutation m = new Mutation(new Path(file).toString());
    m.put(StatusSection.NAME, new Text(tableId.getUtf8Bytes()), StatusUtil.fileCreatedValue(System.currentTimeMillis()));
    BatchWriter bw = ReplicationTable.getBatchWriter(conn);
    bw.addMutation(m);
    bw.flush();
